 ...
 *
 * Gösterilenler:
 *  - 8 adet KPI kartı (Toplam Müşteri, Yönetici, Profil Fotoğraflı, vs.)
 *  - Son 5 kayıt olan kullanıcı tablosu (TC No maskelenmiş)
 *  - Yenile butonu (manual refresh)
 *  - Otomatik dil desteği (TR/EN)
 */
...
export default function AdminPage() {
  ...
  // ─── API'den admin istatistiklerini çek ──────────────────────────────
  // X-User-TC header'ı backend'deki require_admin dependency'si için zorunlu
  const fetchStats = async (silent = false) => {
    ...
    try {
      const res = await fetch(`${API}/api/admin/stats`, {
        headers: {
          'X-User-TC': user.tc_no, // Yetki için admin TC
        },
      });
  ...
  // Tarihi locale'e göre güzel biçimde formatla
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-24 pb-16 flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-primary-50/30">
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-slate-500">{t('admin.loading')}</p>
        </div>
      </div>
    );
  }

  // ─── Hata Durumu (stats null kaldıysa) ──────────────────────────────
  if (!stats) {
    return (
      <div className="min-h-screen pt-24 pb-16 flex items-center justify-center">
        <div className="card-static p-10 text-center max-w-md">
          <Shield className="w-12 h-12 mx-auto mb-4 text-slate-300" />
          <h2 className="text-lg font-bold text-slate-900 mb-2">İstatistikler yüklenemedi</h2>
          <button
            onClick={() => fetchStats()}
  ...
  // ─── KPI Kart Listesi ────────────────────────────────────────────────
  // active_analyses placeholder olarak total_users * 3 (her kullanıcı ~3 analiz)
  // Gerçek bir analysis_logs tablosu olmadığından tahmini değer kullanıyoruz
  const cards = [
    {
      icon: Users,
    ...
    {
      icon: Shield,
      label: t('admin.stat_total_admins'),
    ...
    {
      icon: UserCircle,
      label: t('admin.stat_regular_users'),
    ...
    {
      icon: ImageIcon,
      label: t('admin.stat_with_picture'),
      value: stats.users_with_profile_picture,
      color: 'from-violet-500 to-violet-700',
      accent: 'text-violet-500',
    },
    {
      icon: ClipboardCheck,
      label: t('admin.stat_complete_profile'),
      value: stats.users_with_complete_profile,
      color: 'from-emerald-500 to-emerald-700',
      accent: 'text-emerald-500',
    },
    {
      icon: Clock,
      label: t('admin.stat_last_24h'),
      value: stats.last_24h_registrations,
      color: 'from-pink-500 to-pink-700',
      accent: 'text-pink-500',
    },
    {
      icon: Calendar,
      label: t('admin.stat_last_7d'),
      value: stats.last_7d_registrations,
      color: 'from-cyan-500 to-cyan-700',
      accent: 'text-cyan-500',
    },
    {
      icon: TrendingUp,
      label: t('admin.stat_active_analyses'),
      value: stats.total_users * 3, // Tahmini: her kullanıcı ~3 analiz
  ...
  return (
    <div className="min-h-screen pt-24 pb-16 bg-gradient-to-br from-slate-50 via-white to-primary-50/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 animate-fade-in-up">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">{t('admin.page_subtitle')}</p>
            </div>
          </div>
          <button
        ...
        {/* ─── ADMIN BİLGİ ROZETİ ─── */}
        <div className="mb-8 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 text-amber-800 text-xs font-semibold animate-fade-in-up">
        ...
        <p className="text-center text-xs text-slate-400 mt-8">
          Son güncelleme: {formatDate(stats.generated_at)}
        </p>
      </div>
    </div>
  );
