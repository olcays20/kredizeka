 ...
 * ============================
 * Giriş yapan kullanıcının verilerini API'den çeker ve
 * meslek/adres güncelleme imkanı sunar.
 */
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/profile`, {
        ...
        body: JSON.stringify({ tc_no: user.tc_no, ...editForm }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail);
        ...
        <div className="animate-fade-in-up">
          ...
          <div className="card-static overflow-hidden mb-6">
            ...
            <div className="px-8 pb-8 -mt-12">
              ...
              <h1 className="text-2xl font-extrabold text-slate-900 mt-4">{profile?.full_name || user.full_name}</h1>
              ...
              <div className="space-y-5">
                ...
                <div className="border-t border-slate-100 pt-5 space-y-4">
                  <div>
                    ...
                    {editing ? (
                      <input type="text" value={editForm.occupation} onChange={(e) => setEditForm((p) => ({ ...p, occupation: e.target.value }))} className="input-field" placeholder="Mesleğinizi girin" />
                    ) : (
                      <p className="px-4 py-3 rounded-xl bg-slate-50 text-sm text-slate-700">{profile.occupation || 'Belirtilmemiş'}</p>
                    )}
                  </div>
                  <div>
                    <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 mb-2">
                    ...
                    {editing ? (
                      <textarea value={editForm.address} onChange={(e) => setEditForm((p) => ({ ...p, address: e.target.value }))} className="input-field min-h-[100px] resize-none" placeholder="Adresinizi girin" />
                    ) : (
                      <p className="px-4 py-3 rounded-xl bg-slate-50 text-sm text-slate-700">{profile.address || 'Belirtilmemiş'}</p>
                    )}
                  </div>
                </div>
                ...
                {editing && (
                  <div className="flex gap-3 pt-2">
                    ...
                    <button onClick={() => { setEditing(false); setEditForm({ occupation: profile.occupation || '', address: profile.address || '' }); }} className="btn-secondary">
                      İptal
                    </button>
                  </div>
                )}
